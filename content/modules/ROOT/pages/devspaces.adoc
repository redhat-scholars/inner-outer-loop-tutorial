...
In this lab, we've made changes to the Vote Application frontend in Dev Spaces, and pushed these changes to the Git repository. We've also seen how the pipelines are triggered to rebuild the `vote-ui` application image, and how Argo CD automatically syncs these changes to the `vote-app-dev-{user}` project. In the next lab, we'll deploy the Vote Application to the `vote-app-stage-{user}` staging project, and test the application in a production-like environment.